...
if [ -z "$expect_cells" ]; then
  echo "Error: expect_cells parameter is missing."
  exit 1
fi
...
# Check if the expect_cells exists and is an integar
if ! [[ "$expected_cells" =~ ^[1-9][0-9]*$ ]]; then
  echo "Error: Invalid expected_cells. It should be a positive integer."
  exit 1
fi
...
# Generate and submit CellRanger GEX jobs for all samples
...
cellranger count --id=$job_name --transcriptome=$reference_location --fastqs=../$sequencing_folder/$project_name/ --sample=$job_name --expected-cells=$expected_cells --localcores=2 --localmem=64"
